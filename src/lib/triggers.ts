import { memoize, memoizeWithExpiration } from '@/lib/memoize'
...
class Trie {
	...
	insert(url: string, template: ContextBroTemplate, priority: number) {
		let node = this.root
...
const regexTriggers: Array<{ template: ContextBroTemplate; regex: RegExp; priority: number }> = []
const schemaTriggers: Array<{ template: ContextBroTemplate; pattern: string; priority: number }> =
	[]

let isInitialized = false

export function initializeTriggers(templates: ContextBroTemplate[]): void {
...
const memoizedFindMatchingTemplate = memoizeWithExpiration(
	...
	): Promise<ContextBroTemplate | undefined> => {
		if (!isInitialized) {
