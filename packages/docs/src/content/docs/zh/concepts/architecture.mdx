...
- 当你导航到或聚焦匹配域名的标签页时触发
- 首次提取前等待 10 秒驻留
- 可选的**定时刷新**，按可配置间隔（10秒–1小时）重新提取缓慢变化的页面（信息流、仪表盘）
- 切换标签页时停止
- 使用 `setInterval` —— 无需 Chrome Alarm，因为用户正在主动查看

### 标签页打开模式

使用 Chrome Alarms API 的后台定期捕获：
...
两种模式都使用 SHA-256 内容哈希和可配置的每规则去重窗口，防止发送未变化的内容。
